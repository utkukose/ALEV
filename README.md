...
> **Note:** The environment template is named `env.example` (without a leading dot) so it remains visible in GitHub's file browser and is not accidentally ignored by `.gitignore`. Copy it to `.env` before starting:
> ```bash
> cp env.example .env
> ```
